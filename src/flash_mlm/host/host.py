...
def flash_attn_mlm_precompressed(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    num_heads: int,
    ...
    """Host launcher for pre-packed sequence `_mlm_compressed_kernel`.

    Like `flash_attn_mlm_compressed` but assumes q, k, v are already packed
    and padded 2D tensors, skipping the pack/pad preprocessing step.

    Expected layouts:
      q: [H * total_q_len, D]
      k: [total_q_len, D] if is_mla else [H * total_q_len, D]
      v: [total_q_len, D] if is_mla else [H * total_q_len, D]
      cached k_cache, v_cache:
        - MLA: [total_context_len, D]
        - non-MLA: [H * total_context_len, D]

    Args:
        q: Pre-packed query tensor with layout [H * total_q_len, D].
        k: Pre-packed key tensor. [total_q_len, D] if is_mla, else [H * total_q_len, D].
        v: Pre-packed value tensor. [total_q_len, D] if is_mla, else [H * total_q_len, D].
        num_heads: Number of attention heads (H).
        total_q_len: Total number of (padded) packed query tokens.
        cu_seqlens_q: Cumulative sequence lengths for packed queries [B+1].
        batch_ids_q: Tile-to-batch mapping for query tiles [num_q_tiles].
        q_tile_starts_q: Packed query start offset per tile [num_q_tiles].
        scale: Attention scaling factor applied to QK logits.
        is_mla: Whether to run in MLA latent mode (shared K/V latent layout).
        context_batch_size: Number of context batches represented by cached
            ``cu_seqlens_kv``. If None, inferred from cached values.
        ...
        block_n: Q tile size for the Triton kernel.
        inference_cache: Optional layer-scoped inference cache containing
            prefetched packed context KV. If None, runs with empty context.
        layer_id: Optional layer identifier used to fetch/store prefetched KV
            context when ``inference_cache`` is provided.
        prefill: If True, store k/v as the next cached context for ``layer_id``
            after the kernel launch completes.
        causal_query_seq_attn: If True, each query subsequence can attend
            to packed main tokens from all previous query subsequences in the
            same batch (in addition to cached context).
    ...
    if q.ndim != 2 or k.ndim != 2 or v.ndim != 2:
        raise ValueError("q/k/v must be pre-packed rank-2 tensors")

    d = q.shape[1]
    if k.shape[1] != d or v.shape[1] != d:
        raise ValueError("q/k/v head_dim (dim 1) must match")

    if q.shape[0] != num_heads * total_q_len:
        raise ValueError("q first dim must equal num_heads * total_q_len")

    expected_main_rows = total_q_len if is_mla else num_heads * total_q_len
    if k.shape[0] != expected_main_rows or v.shape[0] != expected_main_rows:
        raise ValueError(
            f"k/v first dim mismatch: expected {expected_main_rows} rows for is_mla={is_mla}"
        )

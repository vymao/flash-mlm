...
def _maybe_compute_q_work_list():
    pass


def _maybe_compute_context_kv_work_list():
    pass
...
def flash_attn_mlm_compressed(
    ...
    if q.ndim != 2 or k.ndim != 2 or v.ndim != 2:
        raise ValueError("q/k/v must be rank-2 packed tensors [H * total_q_len, D]")
    if q.shape != k.shape or q.shape != v.shape:
        raise ValueError("q, k, v must have the same packed shape")
